...
1. **Deterministic parse first** — regex/heuristics extract name, phone, address, district/state, and price directly from the text (labeled fields, phone patterns, Bangla digit conversion, known BD locations, etc.), with no API call needed for well-formatted input.
...
3. **Order creation** — a WooCommerce order is created (`pending` status) with billing/shipping name, phone, address, and matched state; the extracted price and any customer note are attached as order meta/notes for manual review.
...
3. Click **Preview Parsed Data** to check the detected name, phone, address, state, and price before committing.
...
includes/
  ...
  name.php / phone.php / price.php / address.php / location.php / text-utils.php
...
- **Price**: preserved as raw text (e.g. `"1100 + 1000"`) rather than calculated, and stored on the order as `_ai_extracted_price` meta plus an order note, for manual review — line items are not auto-created.
